...
# ── Write side (added 2026-05-18 for Per-toon control isolation) ───────────
...
def write_cc_canonical_keymap(
    ...
    Sets want-Custom-Controls=true, replaces movement bindings in `keymap`
    with the canonical's values, preserves all other keys. Backs up the
    original to <prefs>.ttmt-backup on first call only - subsequent writes
    keep the original intact for future restore.
    Atomic: writes to .ttmt-tmp then renames.
    """
    ...
    try:
        ...
        # Merge: preserve non-movement keymap entries; replace movement.
...
def restore_cc_prefs(prefs_path: Path) -> RestoreResult:
    """Copy <prefs>.ttmt-backup back over <prefs> atomically and remove the backup."""
    prefs_path = Path(prefs_path)
    backup_path = prefs_path.with_suffix(".json.ttmt-backup")
    tmp_path = prefs_path.with_suffix(".json.ttmt-tmp")
...
def detect_custom_bindings(
    settings: CcSettings,
) -> Literal["empty", "stock_wasd_or_arrows", "user_custom"]:
    """Classify the user's movement bindings for UI decision-making."""
    km = settings.keymap or {}
    movement = {a: km.get(a) for a in cc_isolation.MOVEMENT_ACTIONS}

    if all(v is None for v in movement.values()):
        return "empty"
    for stock in _STOCK_BINDINGS.values():
        if all(movement.get(a) == stock[a] for a in cc_isolation.MOVEMENT_ACTIONS):
            return "stock_wasd_or_arrows"
    return "user_custom"
