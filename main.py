...
class MultiToonTool(QMainWindow):
    def __init__(self):
        ...
        self.ctrl_held = False  # Tracks Ctrl state for preset hotkeys
        self.service_running = False
        self.preset_manager = PresetManager()
        self.settings_manager = SettingsManager()

        # Store own window ID so InputService can allow input from this window
    ...
    def on_global_key_press(self, key):
        try:
            # Track Ctrl state for preset hotkeys
            ...
            if normalized:
                self.multitoon_tab.key_event_queue.put(("keydown", normalized))

            # Ctrl+1–5 loads a preset
            if self.ctrl_held and hasattr(key, 'char') and key.char in "12345":
                QMetaObject.invokeMethod(
                    self, "load_preset", Qt.QueuedConnection,
                    Q_ARG(int, int(key.char))
                )
        except Exception:
            pass
    ...
    def on_global_key_release(self, key):
        try:
            ...
            normalized = self.normalize_key(key)
            if normalized:
                self.multitoon_tab.key_event_queue.put(("keyup", normalized))
        except Exception:
            pass

    @staticmethod
    def normalize_key(key):
        """Convert a pynput key into the xdotool keysym string used by InputService."""
