...
def test_builds_ring_in_recency_order_capped_at_8():
    ...
    views = {aid: ("ttr", f"Label {aid}", True) for aid in ordered}
    ...
    assert ring[0] == RingAccount("a0", "ttr", "Label a0", "Toon a0", "dna", True, False)


def test_includes_running_accounts_flagged():
    ring = build_account_ring(
        ["a"],
        account_for=lambda aid: ("cc", "L", True),
...
def test_missing_toon_yields_placeholder_entry():
    ring = build_account_ring(
        ["a"],
        account_for=lambda aid: ("ttr", "L", True),
...
def test_skips_deleted_accounts():
    ring = build_account_ring(
        ["gone", "ok"],
        account_for=lambda aid: None if aid == "gone" else ("ttr", "L", True),
        toon_for=lambda aid: ("T", ""),
        is_running=lambda game, aid: False,
        limit=8,
    )
