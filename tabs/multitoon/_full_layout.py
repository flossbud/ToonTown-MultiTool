...
from PySide6.QtCore import (
    Qt, QSize, QEvent, QTimer, Property, QPropertyAnimation, QEasingCurve,
)
...
# ── Legacy card chrome (only used by this retired full layout) ──────────
# These moved here from _compact_layout.py when that file became the
# pinwheel; the pinwheel does not use a stripe.
...
def _is_descendant_of(widget, ancestor) -> bool:
    """Return True iff `ancestor` is in `widget`'s parent chain. Guards
    mapTo() calls when shared badges/rings are temporarily parented to the
    other layout's card."""
...
class _FullContent(QWidget):
    ...
    def _position_status_rings(self) -> None:
        ...
        populate()."""
# _is_descendant_of is defined at module scope above.
        for i, slot in enumerate(self._card_slots):
