...
> **For whoever picks this up next:** the previous debugging session failed to fix this bug. The user explicitly cut me off after multiple unsuccessful attempts. **Take everything in the "What I believe is the issue" section with a grain of salt** — my track record on this issue is poor. I formed several hypotheses, each looked compelling, each fix made other things better but did not solve the symptom the user is reporting. The data I gathered is reliable; my interpretation of it is what kept being wrong.
